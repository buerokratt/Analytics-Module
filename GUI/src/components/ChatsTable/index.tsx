...
const ChatsTable = (props: Props) => {
  ...
  const chatCommentChangeMutation = useMutation({
    ...
    onSuccess: (res, { chatId, comment }) => {
      const updatedChatList = chats.map((chat) => (chat.id === chatId ? { ...chat, comment } : chat));
  ...
  const chatColumns = useMemo(
    () => [
      ...
      columnHelper.accessor(
        (row) => {
          if (
            Array.isArray(row.allCsaNames) &&
            !(
              row.allCsaNames.length === 1 &&
              (row.allCsaNames[0] == null || row.allCsaNames[0].toString().trim() === '')
            )
          ) {
            ...
            const filteredNames =
              cleanedNames.length > 1 ? cleanedNames.filter((name) => name !== 'Bürokratt') : cleanedNames;
